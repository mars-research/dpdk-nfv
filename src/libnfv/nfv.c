...
void* alloc_1(size_t request ){
	void * addr = buddy_alloc( request,buddy_a);
 printf("addr %p\n",addr);
 return addr; 
}
...
void Load_nf1(){
    ...
    extern int pkey_a;
    CURRENT_DOM = 1;
  	struct elf_domain * elf_a = load_elf_domain("src/libnfv/nf1.so", alloc_1, pkey_a);
 ...
 void Load_nf2(){
    ...
    extern int pkey_b;
    CURRENT_DOM = 2;
  	struct elf_domain * elf_a = load_elf_domain("src/libnfv/nf2.so", alloc_2, pkey_b);
 ...
 void Load_nf3(){
    ...
    extern int pkey_c;
    CURRENT_DOM = 3;
  	struct elf_domain * elf_a = load_elf_domain("src/libnfv/nf3.so", alloc_3, pkey_c);
 ...
 void Load_nf4(){
    ...
    extern int pkey_d;
    CURRENT_DOM = 4;
  	struct elf_domain * elf_a = load_elf_domain("src/libnfv/nf4.so", alloc_4, pkey_d);
